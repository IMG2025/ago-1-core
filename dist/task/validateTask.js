...
export function validateTask(task) {
    const task_id = typeof task === "object" && task !== null && typeof task.task_id === "string"
        ? String(task.task_id)
        : "UNKNOWN_TASK";
    ...
    try {
        ...
        const domDecision = validateDomainForTask(manifest, task_type, scope);
        ...
        const sentinel_policy_id = typeof t.sentinel_policy_id === "string" && t.sentinel_policy_id.trim().length > 0
            ? t.sentinel_policy_id.trim()
            : undefined;
        // 1) Sentinel auth gate
        const authDecision = authorize(sentinel_policy_id
        ...
        // 2) Global scope gate
        const scopeDecision = enforceScope(task_type, scope);
